                        ...
                        try {
                            ...
                            if (passenger.currentCarIndex < 0) {
                                ...
                                for (int i = 0; i < carCount; i++) {
                                    try {
                                        Vec3d carPosFront = ((TrainAccessor) matchedTrain).callGetRoutePosition(i, matchedTrain.spacing);
                                        ...
                                        Vec3d carCenter = new Vec3d(
                                                carPosFront.x - horizontalForward.x * (spacing / 2.0),
                                                carPosFront.y - forward.y * (spacing / 2.0),
                                                carPosFront.z - horizontalForward.z * (spacing / 2.0)
                                        );
                            ...
                            if (passenger.currentCarIndex >= 0 && (carCount == 1 ? passenger.currentCarIndex == 0 : passenger.currentCarIndex < carCount)) {
                                ...
                                // front position of car (TrainClient provides "front" as described)
                                Vec3d carFront = ((TrainAccessor) matchedTrain).callGetRoutePosition(carIdx, matchedTrain.spacing);
                                ...
                                if (forward == null) {
                                    ...
                                    if (yawDegFb != null) {
                                        ...
                                        Vec3d nearestOther = findNearestOtherTrainVector(matchedTrain, carFront);
                                ...
                                // Convert carFront (front tip) to car center by moving backwards by spacing/2 along horizontalForward
                                double halfSpacing = spacing / 2.0;
                                Vec3d center = new Vec3d(
                                        carFront.x - horizontalForward.x * halfSpacing,
                                        carFront.y - forward.y * halfSpacing,
                                        carFront.z - horizontalForward.z * halfSpacing
                                );
                                ...
                                // compute number of cells using LONG_CELL and LAT_CELL
                                int intSpacing = Math.max(1, (int)Math.round(spacing / LONG_CELL));
                                ...
                                // compute start positions (cell centers) so grid is symmetric, using LONG_CELL/LAT_CELL
                                double startLong = -((intSpacing - 1) / 2.0) * LONG_CELL;

                                // --- SHIFT THE LONGITUDINAL GRID START DETERMINISTICALLY PER TRAIN/CAR ---
                                // user requested randomness for the forward grid line positions while keeping lateral 0.5m unchanged.
                                // use a deterministic offset per train/car so it's stable across frames but varies across cars/trains.
                                long offsetSeed = Objects.hash(matchedTrain.id, carIdx, 0x9E3779B97F4A7C15L);
                                Random offsetR = new Random(offsetSeed);
                                double longOffset = (offsetR.nextDouble() - 0.5) * LONG_CELL; // range [-LONG_CELL/2, +LONG_CELL/2)
                                startLong += longOffset;
                                ...
                                // Smoothly move passenger towards target to avoid teleport
